...
def userlog():
    ...
    if request.method == "POST":
        ...
        print('p')
        mydb = mysql.connector.connect(host="localhost",port=3307, user="root", passwd="", database="pavement")
        cursor = mydb.cursor()
    ...
    if request.method == 'POST':
        ...
        if predicted_class1 == 'The model predicts this image has a crack.':
            ...
            if severity_level=="High":
                flash(f'Severity leave is {severity_level}, it will urgently need to fix',"danger")
            elif severity_level=="Medium":
                flash(f'Severity leave is {severity_level}, it will need to fix',"warning")
            else:
                flash(f'Severity leave is {severity_level}, its a cracked but not dangerous so fix it as soon as possible',"primary")
